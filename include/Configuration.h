...
struct BATTERY_ZENDURE_CONFIG_T {
    ...
    uint8_t MinSoC;
    uint8_t MaxSoC;
    enum BypassMode_t : uint8_t { Automatic = 0, AlwaysOff = 1, AlwaysOn = 2 };
    BypassMode_t BypassMode;
    uint16_t MaxOutput;
    bool AutoShutdown;
...
enum GridChargerProviderType { HUAWEI = 0, TRUCKI = 1, VICTRON_IP22 = 3 };
...
struct GRID_CHARGER_VICTRON_CONFIG_T {
    float MaxCurrentA;
};
using GridChargerVictronConfig = struct GRID_CHARGER_VICTRON_CONFIG_T;
...
struct GRID_CHARGER_CONFIG_T {
    ...
    GridChargerVictronConfig Victron;
};
...
public:
    ...
    static void serializeGridChargerVictronConfig(GridChargerVictronConfig const& source, JsonObject& target);
    ...
    static void deserializeGridChargerVictronConfig(JsonObject const& source, GridChargerVictronConfig& target);
